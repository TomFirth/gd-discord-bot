...
const subreddits = ['gamedev', 'IndieGaming', 'indiegames'];
const channelId = config.get('channelIds.unrelated');
const schedule = config.get('schedule.news');
...
export const news = (client) => {
  new CronJob(schedule, async () => {
    const channel = client.channels.cache.get(channelId);
    if (!channel) return;

    const posts = await Promise.all(subreddits.map(getNews));
